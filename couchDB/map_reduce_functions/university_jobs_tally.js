...
var jobs = ['manager', 'surveyor', 'programmer', 'engineer', 'nurse', 'assistant', 'director'];
function(doc){
    ...
    if (point && (inside_box(point, b_uni) || inside_box(point, city_uni) || inside_box(point, aston_uni) || inside_box(point, newman_uni) || inside_box(point, uni_college) ) ) {
        if(doc.meaningcloud.concept_list){
            doc.meaningcloud.concept_list.forEach(function(concept){
                if (concept.text == 'job'){
                    var job = getJob(tweet);
                    if(job != null){
                        emit(job, [1, parseFloat(score)]);
                    }
                }
            });
        }
...
function getJob(text){
    tweet_words = text.toLowerCase().split(" ");
    jobs.forEach(function(job){
        if(tweet_words.indexOf(job)>-1){
            return job;
        }
