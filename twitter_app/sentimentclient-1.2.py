...
#Set up for meaningcloud service, adapted from original client
meaningcloud_host = 'api.meaningcloud.com'
mcloud_api = '/sentiment-1.2.php'
meaningcloud_key = 'd2ba90e4acf476b6ae774ac9931ab0c8'
model = 'en-general' #// es-general/en-general/fr-general

# Auxiliary function to make a post request
def sendPost(text):
    params = urllib.parse.urlencode({"key": meaningcloud_key,"model": model, "txt": text, "src": "sdk-python-1.2"})
    headers = {"Content-type": "application/x-www-form-urlencoded","Accept": "text/plain"}
    conn = http.client.HTTPConnection(meaningcloud_host)
    conn.request("POST", mcloud_api, params, headers)
